...
def connect_to_mail(email_account, password):
    """金鑰"""
    try:
        # 連接到 IMAP 伺服器
...
def search_and_extract_emails( email_account, password,start_time):
    # access_token = credentials.token
    # auth_string = f"user={user_email}\1auth=Bearer {access_token}\1\1"

    # mail = imaplib.IMAP4_SSL('imap.gmail.com')
    # mail.authenticate('XOAUTH2', lambda x: auth_string)
    mail = connect_to_mail(email_account, password)
    # 選擇收件匣
    mail.select('inbox')
        ...
        for num in mail_ids:
            ...
            if result == 'OK':
                ...
                if time < loc_dt_format:
                    ...
                    return gcal(event, formatted_date+"/"+formatted_date, place)
                else:
